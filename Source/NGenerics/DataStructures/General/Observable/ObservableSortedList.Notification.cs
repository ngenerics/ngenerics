...
{
    /// <summary>
    /// An observable version of the <see cref="SortedList{T}"/> data structure.
    /// </summary>
    /// <typeparam name="T">The type of elements in the sorted list.</typeparam>
    public partial class ObservableSortedList<T> : INotifyCollectionChanged, INotifyPropertyChanged
    {
        #region Globals
        ...
        /// <inheritdoc />
        public ObservableSortedList(int capacity, IComparer<T> comparer): base(capacity,comparer)
        {
            monitor = new SimpleMonitor();
        }
