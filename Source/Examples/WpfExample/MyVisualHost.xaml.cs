...
public class MyVisualHost : FrameworkElement
{
    ...
    {
        double barWidth = (chartWidth - (_axis_start * 3)) / (barCount);
    ...
    public void CreateBarChart(ICollection dt, double _maxData, double chartHeight, double chartWidth)
    {
        ...
        double barWidth = SetBarWidth(dt.Count, chartWidth);
        ...
        using (DrawingContext dc = visual.RenderOpen())
        {
            ...
            {
                // Calculate bar value.
                double height = Convert.ToDouble(el) * (chartHeight - _axis_start) / _maxData;
                left = right ;
                right = barWidth + left;

                dc.DrawRectangle(null, _chartPen, new Rect(left, 50 - height - _axis_start, barWidth, height));
            }
            stopWatch.Stop();
            ...
            string elapsedTime = String.Format("{0} - {1} - {2}", MethodBase.GetCurrentMethod().Name, dt.Count, stopWatch.ElapsedTicks);
    ...
    public void CreateDotChartGeometryAlt(ICollection dt, double _maxData, double chartHeight, double chartWidth)
    {
        ...
        using (var geoContext = geo.Open())
        {
            ...
            foreach (var el in dt)
            {
                ...
                geoContext.BeginFigure(new Point(centerX, 50 - height - _axis_start), false, false);
                geoContext.LineTo(new Point(centerX+2, 50 - height - _axis_start),true,false);
            }    
        }
    ...
    public void CreateDotChartGeometry(ICollection dt, double _maxData, double chartHeight, double chartWidth)
    {
        ...
        foreach (var el in dt)
        {
            ...
            ellipses.Children.Add(
                                     new EllipseGeometry(new Point(centerX, 50 - height - _axis_start), 1, 1)
                                );
        }
        ellipses.Freeze();
    ...
    public void CreateDotChartAlt(ICollection dt, double _maxData, double chartHeight, double chartWidth)
    {
        ...
        using (DrawingContext dc = visual.RenderOpen())
        {
            ...
            foreach (var el in dt)
            {
                ...
                double height = Convert.ToDouble(el) * (chartHeight - _axis_start) / _maxData;
                centerX += barWidth;
                dc.DrawRectangle(null, _chartPen, new Rect(centerX, 50 - height - _axis_start ,1, 1));
            }
            stopWatch.Stop();
    ...
    public void CreateDotChart(ICollection dt, double _maxData, double chartHeight, double chartWidth)
    {
        ...
        using (DrawingContext dc = visual.RenderOpen())
        {
            ...
            foreach (var el in dt)
            {
                ...
                double height = Convert.ToDouble(el) * (chartHeight - _axis_start) / _maxData;
                centerX += barWidth;
                dc.DrawEllipse(null, _chartPen, new Point(centerX, 50 - height - _axis_start), 1, 1);
